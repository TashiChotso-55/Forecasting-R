...
It is clear form the above MAPE values that the model preferred for modelling is the Mean Model.

```{r}
s1_mean_fc <- s1_mean_fit %>% forecast(h = no_of_test_records1+30)
```
...
```{r}
s2_mean_fc <- s2_mean_fit %>% forecast(h = no_of_test_records+30)
```
...
df_s <- data.frame(id = c(1:60),
                 no_of_applicants = c(tail(s1_mean_fc$.mean,30), tail(s2_mean_fc$.mean,30))
                 )
